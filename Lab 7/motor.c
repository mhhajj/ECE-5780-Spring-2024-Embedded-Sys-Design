...
void encoder_init(void) {
    ...
    GPIOB->MODER &= ~(GPIO_MODER_MODER4_0 | GPIO_MODER_MODER4_1);
...
void TIM6_DAC_IRQHandler(void) {
    motor_speed = (TIM3->CNT - 0x7FFF); // Calculate the motor speed in raw encoder counts
    TIM3->CNT = 0x7FFF; // Reset back to center point

    PI_update(); // Call the PI update function
...
CR |= ADC_CR_ADSTART; // Signal conversion start
}
...
void PI_update(void) {
   ...
   pwm_setDutyCycle(output);
    ...
    // Read the ADC value for current monitoring, actual conversion into meaningful units will be performed by STMStudio
